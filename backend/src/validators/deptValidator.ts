...
export default class DeptValidator extends BaseValidator {
  ...
  validateCreateBody: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!deptName) {
      Logger.error('addNewDept: validateCreateBody: ' + Status.SERVER_ERRORS.reviews.name_not_found);
      this.sendError(res, Status.ERROR_CODES.reviews.name_not_found_msg);
      return;
    }

    if (typeof deptName != 'string') {
      Logger.error('addNewReview: validateCreateBody: ' + Status.SERVER_ERRORS.reviews.name_type);
      this.sendError(res, Status.ERROR_CODES.reviews.name_type_msg);
      return;
    }
